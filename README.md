...
- 🧠 **ИИ система**: OpenRouter (упрощено, удалены Groq/OpenAI/Hugging Face/Polza)
...
Используется единый провайдер: **OpenRouter** (доступны бесплатные модели, работает в РФ).
...
### Проблемы с ИИ:
- Проверьте `OPENROUTER_API_KEY` в `ТОКЕНЫ.env`
